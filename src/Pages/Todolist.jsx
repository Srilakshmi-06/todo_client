...
const Todolist = () => {
    ...
    const postTodo = async () => {
        try {
            ...
            await axios.post("https://todo-server-ec2-e68x.onrender.com/csbs/addtodo", { todo });
            setTodo('')
            getTodo()
            setStatus(true)
    ...
    //for get operation
    const getTodo = async () => {
        await axios.get("https://todo-server-ec2-e68x.onrender.com/csbs/gettodo")
            .then((response) => {
    ...
    const deleteTodo = async (id) => {
        try {
            await axios.delete(`https://todo-server-ec2-e68x.onrender.com/csbs/deletetodo/${id}`)
            getTodo()
        }
        catch (err) {
            console.error(err)
    ...
    try {
        await axios.put(
            `https://todo-server-ec2-e68x.onrender.com/csbs/updatetodo/${id}`,
            { todo: data }
        );
        getTodo();
    } catch (err) {
